...
module.exports = (req, res) => {
  let channelAccessToken = functions.config()['pps-stretch-bot']['line']['channel-access-token'];
  let channelSecret = functions.config()['pps-stretch-bot']['line']['channel-secret'];
  ...
  if (req.method === 'POST') {
    ...
    body.events.map(event => {
      if (event.type === 'message') {
        if (event.message.type === 'text') {
          if (textMapping[event.message.text]) {
            let msg = textMapping[event.message.text].text ||
                event.message.text;
            publishMqtt({topic: 'stretch/$/command', msg});
          } else {
            publishMqtt({topic: 'stretch/$/command', msg: event.message.text});
          }
        ...
        res.status(200).send('stretch GET OK ' + JSON.stringify(req));
      } else {
